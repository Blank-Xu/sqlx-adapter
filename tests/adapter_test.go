...
func testTableName(t *testing.T, db *sqlx.DB) {
	_, err := sqlxadapter.NewAdapter(db, "")
	if err != nil {
		t.Fatalf("sqlxadapter.NewAdapter failed, err: %v", err)
	}
}

func testSql(t *testing.T, db *sqlx.DB, tableName string) {
	a, err := sqlxadapter.NewAdapter(db, tableName)
	if err != nil {
		t.Fatalf("sqlxadapter.NewAdapter failed, err: %v", err)
	}

	test := sqlxadapter.Test{
		Adapter: a,
	}
	...
	if b := test.IsTableExist(); b == false {
		t.Fatal("IsTableExist test failed")
	}

	if err = test.InsertRow(lines[0]); err != nil {
	...
	if err = test.DeleteByArgs(lines[8]); err != nil {
	...
	records, err = test.SelectWhereIn(filter)
	if err != nil {
	...
	if err = test.TruncateTable(); err != nil {
...
func testPolicyLine(t *testing.T, db *sqlx.DB, tableName string) {
	...
	testLine := sqlxadapter.CasbinRule{
		PType: "p",
		V0:    "test0",
		V1:    "test1",
		V2:    "test2",
	...
	line := test.GenPolicyLine("p", rule)
	...
	test.LoadPolicyLine(&line, e.GetModel())
}
...
func initPolicy(t *testing.T, db *sqlx.DB, tableName string) {
	...
	a, err := sqlxadapter.NewAdapter(db, tableName)
	if err != nil {
		t.Fatal("sqlxadapter.NewAdapter test failed, err: ", err)
	}
...
func testSaveLoad(t *testing.T, db *sqlx.DB, tableName string) {
	...
	a, _ := sqlxadapter.NewAdapter(db, tableName)
